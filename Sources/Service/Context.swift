...
class ContextService {
    ...
    static func getInputContent() -> String? {
        ...
        guard result == .success, let content = value as? String else {
            log.warning("Cannot get element value: \(result.rawValue)")
            return nil
        }
    ...
    static func getSelectedText() async -> String? {
        ...
        {
            var value: CFTypeRef?
            if AXUIElementCopyParameterizedAttributeValue(element, kAXStringForRangeParameterizedAttribute as CFString, range, &value) == .success,
